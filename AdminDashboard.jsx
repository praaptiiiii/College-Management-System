import React from "react";

const AdminDashboard = () => {
  const handleAddStudents = () => {
    // Handle add/delete students logic
    console.log("Add Students");
  };
  const handleDeleteStudents = () => {
    // Handle add/delete students logic
    console.log("Delete Students");
  };

  const handleAddTeachers = () => {
    // Handle add/delete teachers logic
    console.log("Add Teachers");
  };
  const handleDeleteTeachers = () => {
    // Handle add/delete students logic
    console.log("Delete Teachers");
  };
  const handleEditStudentDetails = () => {
    // Handle edit personal details logic
    console.log("Edit Student Details");
  };
  const handleEditTeacherDetails = () => {
    // Handle edit personal details logic
    console.log("Edit Teacher Details");
  };

  return (
    <div className="admin-dashboard">
      <h1>Admin Dashboard</h1>
      <div className="dashboard-option">
        <button onClick={handleAddStudents} className="dashboard-option">
          <img
            src="data:image/png;base64,"
            alt=""
          />
          Add Student
        </button>

        <button onClick={handleDeleteStudents} className="dashboard-option">
          <img
            src="data:image/png;base64,"
            alt=""
          />
          Delete Student
        </button>

        <button onClick={handleEditStudentDetails} className="dashboard-option">
          <img
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSL3WdPBU4F0jjWIefAnJGvIYgqefCKYq_fpQ&usqp=CAU"
            alt="Personal Details Icon"
          />
          Edit Student Details
        </button>

        <button onClick={handleAddTeachers} className="dashboard-option">
          <img
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcScn6y4tTWQO8SHR3jssujOZ5aStrnP8Si6sQ&usqp=CAU"
            alt=""
          />
          Add Teacher
        </button>

        <button onClick={handleDeleteTeachers} className="dashboard-option">
          <img
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcScn6y4tTWQO8SHR3jssujOZ5aStrnP8Si6sQ&usqp=CAU"
            alt=""
          />
          Delete Teacher
        </button>

        <button onClick={handleEditTeacherDetails} className="dashboard-option">
          <img
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSL3WdPBU4F0jjWIefAnJGvIYgqefCKYq_fpQ&usqp=CAU"
            alt="Personal Details Icon"
          />
          Edit Teacher Details
        </button>
      </div>
    </div>
  );
};

export default AdminDashboard;
